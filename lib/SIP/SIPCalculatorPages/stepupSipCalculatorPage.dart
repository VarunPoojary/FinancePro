...
class stepupSipCalculatorPage extends StatefulWidget {
  const stepupSipCalculatorPage({Key? key}) : super(key: key);

  @override
  State<stepupSipCalculatorPage> createState() => stepupSipCalculatorPageState();
}

class stepupSipCalculatorPageState extends State<stepupSipCalculatorPage> {
  final TextEditingController monthlyInvestmentController = TextEditingController(text: '25000');
  final TextEditingController annualIncreaseController = TextEditingController(text: '10');
  ...
  void calculateTotalValue() {
    ...
    double annualIncrease = double.parse(annualIncreaseController.text) / 100;

    double A = 0.0;
    double invested = 0.0;

    for (int i = 0; i < n; i++) {
      double currentInvestment = P * pow(1 + annualIncrease, (i / 12).floor());
      A += currentInvestment * pow(1 + r, n - i);
                    ...
                    child: Row(
                      children: [
                        GestureDetector(
                          ...
                          onTap: () {
                            int currentValue = int.parse(annualIncreaseController.text);
                            setState(() {
                              currentValue -=1;
                              if(currentValue<=1)
                                currentValue=1;
                              annualIncreaseController.text = (currentValue)
                                  .toString(); // incrementing value
                            });
                          },
                        ),
                        ...
                        Container(
                          ...
                          child: TextField(
                            ...
                            textAlign: TextAlign.center,
                            controller: annualIncreaseController,
                            obscureText: false,
                            keyboardType: TextInputType.number,
                            decoration: InputDecoration(
                              border: InputBorder.none,
