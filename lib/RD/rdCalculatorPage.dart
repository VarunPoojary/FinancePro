...
class rdCalculatorPage extends StatefulWidget {
  const rdCalculatorPage({Key? key}) : super(key: key);

  @override
  State<rdCalculatorPage> createState() => rdCalculatorPageState();
}

class rdCalculatorPageState extends State<rdCalculatorPage> {
  ...
  void calculateTotalValue() {
    double monthlyInvestment = double.parse(totalInvestmentController.text);
    double annualRate = double.parse(estimatedReturnController.text);
    int years = int.parse(timePeriodController.text);

    double totalInvestment = monthlyInvestment*years*12;
    int n = years * 12; // Number of months
    double r = annualRate / 12 / 100; // Monthly interest rate

    // double maturityAmount = monthlyInvestment * (pow((1 + r), n) - 1) / r * (1 + r);
    double maturityAmount = 0;
    for (int month = 1; month <= years * 12; month++) {
      double amount = monthlyInvestment * pow((1 + annualRate / (100 * 4)), 4 * (years - (month - 1) / 12));
